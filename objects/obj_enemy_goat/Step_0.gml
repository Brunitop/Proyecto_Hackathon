...
if (state == "patrolling") {
	...
	if (node_count > 0) {
		...
		if (is_moving) {
			...
			var attempt = ds_grid_get(global.attempt_grid, target_x, target_y);
		    ...
		    if (occupant == 0 && attempt == id) {
				if (progress >= 1) {
					...
					ds_grid_set(global.attemptgrid, xpos, ypos, 0); // Mark the attempt as finished
		    ...
		    if (global.global_tick == 1) {
				...
				if (patrol_pause_timer <= 0) {
					...
					if (!is_in_patrol_area) {
					    ...
					    ds_grid_set(global.attempt_grid, xpos, ypos, 0);
